#!/usr/bin/env python3
"""
OmniSVG Base Model Inference Script for Test Holdout Dataset (Multi-GPU)
========================================================================
与 inference_hvm_s1_test.py 使用完全相同的数据集和样本索引，
输出格式一致（sample_XXXX_base.svg），方便对比计算指标。

区别：不加载 HVM 模块，直接用 OmniSVG 基模生成。

用法示例:
CUDA_VISIBLE_DEVICES=4,5,7 python inference/inference_omnisvg_base_test.py \
...
def load_base_model(
    ...
    print(f"[{device}][1/3] Loading tokenizer from {base_model_path} ...")
    ...
    print(f"[{device}][2/3] Loading OmniSVG {model_size} base model ...")
...
# ============================================================================
# Input preparation (与 HVM 脚本完全一致)
# ============================================================================
...
# ============================================================================
# Generation & decoding (与 HVM 脚本完全一致)
# ============================================================================
...
def run_on_single_gpu(
    ...
    # --- Load base model (no HVM) ---
    base_model, tokenizer, processor, token_config = load_base_model(
    ...
    for idx in pbar:
        ...
        if args.resume:
            expected_tags = [tag]
            all_exist = all(
                (output_dir / f"sample_{idx:04d}_{t}.svg").exists()
        ...
        if candidates:
            ...
            for ci, cand in enumerate(candidates):
                ...
                base_name = f"sample_{idx:04d}_{tag}{suffix}"
                ...
                if args.save_png:
                    ...
                    if img is not None:
                        img.save(str(output_dir / f"{base_name}.png"))

                if args.save_tokens:
                    tok_path = output_dir / f"{base_name}_tokens.json"
...
def parse_args():
    p = argparse.ArgumentParser(description="OmniSVG Base Model Inference for Test Holdout (Multi-GPU)")
    ...
    p.add_argument("--hvm_dir", type=str,
                   ...
                   help="HVM 预计算特征目录 (仅用于加载 dataset metadata)")
    ...
    # output
    p.add_argument("--output_dir", type=str, default="./inference_results/omnisvg_base_test")
...
def main():
    ...
    print("=" * 70)
    print(f"OmniSVG Base Inference (Test Holdout)  --  Multi-GPU Data Parallel")
    print("=" * 70)
    print(f"  Model size       : {args.model_size}")
    print(f"  Mode             : OmniSVG Base (no HVM)")
    print(f"  OmniSVG ckpt     : {args.omnisvg_checkpoint}")
