...
CUDA_VISIBLE_DEVICES=0,1,2,3,6 python inference/inference_hvm_multigpu.py \
    --hvm_checkpoint outputs_s1_fixed0.03/hvm_step_8000.pt \
    --sample_indices $(seq 0 999) \
    --output_dir inference/output_s1_fixed0.03_8000step_02_27_11_59 \
...
def run_on_single_gpu(
    local_rank: int,        # 本进程使用的 GPU 编号（在 CUDA_VISIBLE_DEVICES 范围内）
    gpu_id: int,            # 实际 CUDA device id（= local_rank）
    sample_indices: List[int],  # 本进程负责的样本下标
    args: argparse.Namespace,
):
    """
    在单张 GPU 上独立跑推理，不与其他进程通信。
    """
...
def worker(local_rank: int, args: argparse.Namespace, index_splits: List[List[int]]):
    ...
    run_on_single_gpu(
        ...
        gpu_id=local_rank,           # CUDA_VISIBLE_DEVICES 已经做了重映射
...
def main():
    ...
    if num_gpus == 1:
        ...
        # 多卡：spawn 子进程，每个子进程绑定一张卡，互不通信
        # 注意：使用 spawn 而非 fork，避免 CUDA 上下文继承问题
        mp.start_processes(
            worker,
