#!/usr/bin/env python3
"""
OmniSVG Base Model Inference Script
Adapted for current environment (no vllm, no qwen_vl_utils)
"""
...
# Add project root to path to import decoder
...
# Try to import qwen_vl_utils, but handle failure
try:
    from qwen_vl_utils import process_vision_info
    HAS_QWEN_VL_UTILS = True
except ImportError:
    HAS_QWEN_VL_UTILS = False
    print("Warning: qwen_vl_utils not found. Image-to-SVG task might not work correctly.")

# Import SVGTokenizer from current directory (inference/)
try:
    from tokenizer import SVGTokenizer
except ImportError:
    # If running from root, inference.tokenizer might be needed
    from inference.tokenizer import SVGTokenizer

# Load config
CONFIG_PATH = os.path.join(os.path.dirname(__file__), 'config.yaml')
if not os.path.exists(CONFIG_PATH):
    # Fallback to current directory if running from inference/
...
DEFAULT_TEXT_DATASET_PATH = "/mnt/data/wuqingman/datasets/OmniSVG/MMSVGBench/data/text2svg-00000-of-00001.parquet"
DEFAULT_QWEN_MODEL_PATH = "/mnt/data/wuqingman/models/Qwen/Qwen2.5-VL-7B-Instruct"
DEFAULT_OMNISVG_WEIGHT_PATH = "/mnt/data2/wuqingman/models/OmniSVG/OmniSVG1.1_8B/pytorch_model.bin"
DEFAULT_DATASET_TASK_TYPE = "text2svg"
...
def parse_args():
    ...
    parser.add_argument('--resume', action='store_true', default=False,
                        help='Resume from previous run by skipping existing files')
    # Input/Output
    parser.add_argument('--input', type=str, default=None,
                        help='Input file (.txt or .parquet for text-to-svg) or directory (for image-to-svg). '
                             'Defaults to MMSVGBench parquet when --task text-to-svg and available.')
    parser.add_argument('--output', type=str, default='./output',
                        help='Output directory for generated SVGs')
    parser.add_argument('--dataset-task-type', type=str, default=DEFAULT_DATASET_TASK_TYPE,
                        help=f"Task type filter applied when reading parquet inputs (default: {DEFAULT_DATASET_TASK_TYPE}). "
    ...
    parser.add_argument('--model-path', type=str, default=None,
                        help='Local path or HuggingFace repo ID for Qwen model (overrides config). '
                             'Defaults to local Qwen2.5-VL-7B-Instruct when available.')
    parser.add_argument('--weight-path', type=str, default=None,
                        help='Local path or HuggingFace repo ID for OmniSVG weights (overrides config). '
                             'Defaults to the provided OmniSVG1.1_8B weights when available.')
    ...
    if not args.input:
        if args.task == 'text-to-svg':
            if os.path.exists(DEFAULT_TEXT_DATASET_PATH):
                args.input = DEFAULT_TEXT_DATASET_PATH
                print(f"[Info] Using default dataset: {DEFAULT_TEXT_DATASET_PATH}")
            else:
                parser.error("Input file is required for text-to-svg when the default MMSVGBench dataset "
                             "is not accessible. Please provide --input.")
        else:
            parser.error("--input is required for image-to-svg tasks.")
    
    if args.dataset_task_type is not None:
        args.dataset_task_type = args.dataset_task_type.strip()
        if not args.dataset_task_type:
            args.dataset_task_type = None
    
    if args.model_path is None and os.path.exists(DEFAULT_QWEN_MODEL_PATH):
        args.model_path = DEFAULT_QWEN_MODEL_PATH
        print(f"[Info] Using default Qwen model path: {DEFAULT_QWEN_MODEL_PATH}")
    
    if args.weight_path is None and os.path.exists(DEFAULT_OMNISVG_WEIGHT_PATH):
        args.weight_path = DEFAULT_OMNISVG_WEIGHT_PATH
        print(f"[Info] Using default OmniSVG weights: {DEFAULT_OMNISVG_WEIGHT_PATH}")
    
    return args


def load_text_prompts(input_path, task_filter=None):
    """Load text prompts (and optional metadata) from a .txt or .parquet source."""
    ...
    if suffix == '.parquet':
        # Try using pyarrow directly if pandas is missing
        try:
            ...
            if 'text' in column_names:
                ...
                raise ValueError(f"Parquet file must contain 'text', 'description', or 'caption' column. Found: {column_names}")
            
            # Read text column
            texts = table.column(text_col).to_pylist()
            
            # Read metadata columns if available
            ids = table.column('id').to_pylist() if 'id' in column_names else [None] * len(texts)
            task_types = table.column('task_type').to_pylist() if 'task_type' in column_names else [None] * len(texts)
            
            for i, text in enumerate(texts):
                if task_filter and task_types[i] and task_types[i] != task_filter:
                    continue
                ...
                if ids[i] is not None:
                    metadata['id'] = ids[i]
                if task_types[i] is not None:
                    metadata['task_type'] = task_types[i]
                    
                prompts.append({'text': str(text).strip(), 'metadata': metadata or None})
            ...
            if pd is None:
                raise ImportError(
                    "Reading parquet datasets requires pyarrow or pandas. "
                    "Please install one of them."
                )
            # Fallback to pandas logic if pyarrow failed but pandas might be available (though unlikely here)
            pass

        if pd is None:
            raise ImportError(
                "Reading parquet datasets requires pandas (and pyarrow/fastparquet). "
                "Please install the dependencies to continue."
            )
        
        columns_priority = ['text', 'description', 'caption', 'id', 'task_type', 'type', 'url']
        columns_to_read = None
        
        try:
            import pyarrow.parquet as pq
            schema_cols = set(pq.read_schema(input_path).names)
            if 'text' not in schema_cols:
                raise ValueError("Parquet file must contain a 'text' column for prompts.")
            columns_to_read = [col for col in columns_priority if col in schema_cols]
        except ImportError:
            columns_to_read = None
        except ValueError:
            raise
        ...
        if task_filter and 'task_type' in df.columns:
        ...
        meta_fields = [field for field in ('id', 'task_type', 'type', 'url') if field in available_cols]
        ...
        for row in df.itertuples(index=False):
            ...
            for field in meta_fields:
                metadata[field] = getattr(row, field, None)
            
            metadata = {k: v for k, v in metadata.items() if v not in (None, '')}
            prompts.append({'text': text_value, 'metadata': metadata or None})
    ...
    for idx, line in enumerate(lines, start=1):
        prompts.append({'text': line, 'metadata': {'source_index': idx}})
    
    return prompts


def build_output_basename(prompt_text, index, metadata=None):
    """Generate a deterministic base filename that includes dataset ids when available."""
    sample_id = None
    
    if metadata:
        dataset_id = metadata.get('id')
        if dataset_id:
            dataset_id = "".join(c if c.isalnum() or c in {'-', '_'} else '_' for c in str(dataset_id))
            sample_id = dataset_id.strip('_')
    
    if not sample_id:
        safe_prompt = "".join(
            c if c.isalnum() or c in {'-', '_', ' '} else '_' 
            for c in prompt_text[:50]
        )
        safe_prompt = "_".join(safe_prompt.strip().split())
        sample_id = safe_prompt.strip('_') or f"sample_{index+1:04d}"
    
    return f"{index+1:04d}_{sample_id}"
...
def load_models(model_size: str, weight_path: str = None, model_path: str = None):
    ...
    device_map = (
        "auto" if torch.cuda.is_available() and torch.cuda.device_count() > 1
        else ("cuda" if torch.cuda.is_available() else {"": "cpu"})
    )
    print(f"Device map: {device_map}")
    
    # Get vocab size and other params from config
    vocab_size = get_config_value(model_size, 'model', 'vocab_size')
    bos_token_id = get_config_value(model_size, 'model', 'bos_token_id')
    eos_token_id = get_config_value(model_size, 'model', 'eos_token_id')
    pad_token_id = get_config_value(model_size, 'model', 'pad_token_id')
    
    # Fallback to defaults if not in model specific config
    if bos_token_id is None: bos_token_id = BOS_TOKEN_ID
    if eos_token_id is None: eos_token_id = EOS_TOKEN_ID
    if pad_token_id is None: pad_token_id = PAD_TOKEN_ID
...
- Maintain visual clarity and composition"""
        ...
        if not HAS_QWEN_VL_UTILS:
             raise ImportError("qwen_vl_utils is required for image-to-svg task but not found.")
...
def save_results(candidates, output_dir, base_name, save_png=False, save_all=False,
                 prompt_text=None, prompt_metadata=None):
    """Save generated SVG(s), optional PNG(s), and the source text prompt."""
...
def process_text_to_svg(args):
    """Process text-to-svg task with resume support"""
    input_path = args.input
    
    try:
        prompt_entries = load_text_prompts(input_path, task_filter=args.dataset_task_type)
    except Exception as exc:
        print(f"Error loading prompts from {input_path}: {exc}")
        return
    
    if not prompt_entries:
        print("Error: No prompts found in the provided input source")
        return
    
    print(f"\nFound {len(prompt_entries)} prompts to process")
    ...
    if args.resume and os.path.exists(args.output):
        ...
        for fname in os.listdir(args.output):
            if fname.endswith('.svg'):
                # Extract base name (without .svg suffix)
                base = fname.rsplit('.', 1)[0]
                existing_files.add(base)
    ...
    for idx, entry in enumerate(prompt_entries):
        ...
        # Build filename and check if exists
        safe_name = build_output_basename(prompt, idx, metadata)
        svg_path = os.path.join(args.output, f"{safe_name}.svg")
        
        if args.resume and os.path.exists(svg_path):
            print(f"\n[{idx+1}/{len(prompt_entries)}] ⏭️  Skipping (already exists): {safe_name}")
            total_skipped += 1
            continue
        
        dataset_id = metadata.get('id')
        id_suffix = f" (id: {dataset_id})" if dataset_id else ""
        preview = prompt[:80]
        ellipsis = '...' if len(prompt) > 80 else ''
        print(f"\n[{idx+1}/{len(prompt_entries)}] Processing{id_suffix}: {preview}{ellipsis}")
    ...
    print(f"  Success: {total_success}/{len(prompt_entries)}")
    print(f"  Failed: {total_failed}/{len(prompt_entries)}")
    print(f"  Skipped: {total_skipped}/{len(prompt_entries)}")
...
def process_image_to_svg(args):
    ...
    if not HAS_QWEN_VL_UTILS:
        print("Error: qwen_vl_utils is required for image-to-svg task but not found.")
        return
