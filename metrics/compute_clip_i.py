#!/usr/bin/env python3
"""计算 CLIP-I (CLIP image-image cosine similarity vs GT, ↑ better)"""
...
def get_model(device, model_path):
    ...
    if _MODEL is None:
        ...
        print(f"[CLIP-I] Model loaded from {model_path}")
    return _MODEL, _PROCESSOR


@torch.no_grad()
def compute_clip_i(gen_pil, gt_pil, device, model_path):
...
def main():
    ...
    print(f"[CLIP-I] total={len(all_gt_indices)}, valid={len(valid_indices)}, failed={len(failed_indices)}"
          + (f", skip={len(done_indices)}, todo={len(todo_valid)}" if args.resume else ""))

    per_sample = list(existing_per_sample)
    for idx in tqdm(todo_valid, desc="CLIP-I"):
        info = samples[idx]
        ...
        for cand_path in info["candidates"]:
            ...
            if gen_pil is None:
                ...
                continue
            values.append(compute_clip_i(gen_pil, gt_pil, args.device, args.clip_model))
